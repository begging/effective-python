...
'''
get(): blocking when there's no input
put(): blocking when queue is full.
'''
...
'''
If you use .join() and don't call .task_done() for every processed item, your
script will hang forever.

This lets you wait for a phase’s input queue to drain and eliminates the need
for polling the done_queue at the end of your pipeline.
'''
in_queue = Queue()
def consumer():
    print('Consumer waiting')
